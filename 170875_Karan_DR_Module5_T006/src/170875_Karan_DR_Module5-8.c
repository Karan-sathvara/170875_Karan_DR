/*
* File Name     : 170875_Karan_DR_Module5-8.c
* Description   : There is no error checking in day_of_year or month_day.
*               : Remedy this defect.
...
void month_day(int16_t year, int16_t yearday, int8_t *pmonth, int16_t *pday)
{
	...
	*pday = -1;
    ...
    for (idx = 1; idx <= 12 && yearday > daytab[leap][idx]; idx++){
        yearday -= daytab[leap][idx];
    }
